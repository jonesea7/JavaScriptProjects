...
function updateCalcScreen() {
    ...
    calcButtons.addEventListener('click', (event) => {
        ...
        if (elementClicked.classList.contains('operator')) {
            useOperator(elementClicked.value);
            updateCalcScreen();
            return;
        }

        if (elementClicked.classList.contains('decimal-function')) {
            inputDecimal(elementClicked.value);
            updateCalcScreen();
            return;
        }

        if (elementClicked.classList.contains('all-clear-function')) {
            calculatorReset();
            updateCalcScreen();
            return;
        }

        inputDigit(elementClicked.value);
        updateCalcScreen();
    })
};